...
export default class ShopdetailPage extends Component {
  ...
  componentDidMount() {
    axios
      .get(`/api/shop/${this.props.match.params.shopid}`)
      .then(result => {
        if (result.status === 200 && result.data.length > 0) {
          ...
          axios
            .get(`/api/shop/menu/${this.props.match.params.shopid}`)
            .then(result2 => {
              ...
              axios
                .get(
                  `/api/userProfile/comment/${this.props.match.params.shopid}`
                )
                .then(result3 => {
  ...
  render() {
    ...
    return <div>{this.state.isLoading ? loader : renderObj}</div>;
  }
}
